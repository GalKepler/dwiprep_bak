...
def fetch_additional_files(nii: Path, parameter: str):
    """
    Searches for json, .bvec and .bval files corresponding to a list of NIfTI files' paths.
    Parameters
    ----------
    inputs : list
    ...
    Returns
    -------
    [list or PathLike object]
        [Either a single or multiple PathLike objects representing existing .json/.bvec/.bval files]

    Raises
    ------
    FileNotFoundError
        [If a corresponding .json/.bvec/.bval file does not exist under the same directory as the NIfTI, notify the user.]
    """
    nii = Path(nii)
